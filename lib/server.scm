...
(define (lys:debug msg)
  (begin
    (display (format #f "~a (~a): " (strftime "%Y-%m-%d %H:%M:%S" (localtime (current-time))) (getpid)) lys:output-port)
...
(define (lys:start-server . args)
  (let* ((port (if (null? args) lys:default-port (car args)))
    ...
    (newline lys:output-port)
    (lys:debug (format #f "Listening on port ~a" port))
    (flush-all-ports)

    (while #t
      (let* ((connection (accept server-socket))
             (client-socket (car connection)))
        (lys:fork-worker (lambda () (lys:client-handler client-socket)))
